...
\goiaba{}\footnote{Additional information and link were also omitted
  to keep anonymity.} is developed in Common Lisp
\cite{graham94:lisp}, with the Steel Bank Common Lisp (SBCL) compiler
\cite{team07:sbcl}.

In \goiaba{}, contours are represented symbolicaly in two ways: as
simple contours and contours with durations. Simple contours
represents only contour element values: \code{(5 9 6)}. Contours with
duration represents also the time when the element occur: \code{((0
  5)(1 9)(2 6))}. Although \goiaba{} provides a codification that
